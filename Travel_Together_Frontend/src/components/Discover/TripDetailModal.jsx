...
export default function TripDetailModal({ trip, onClose, onSave, onShare, onAskOrganizer }) {
  ...
  const [joinState,  setJoinState]  = useState(() => {
    // my_status may be a string (list) or { role, status } (detail)
                ...
                <div className="mb-[18px]">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {(trip.highlights || []).map((h, i) => (
                      <div key={i} className="flex items-center gap-2.5 bg-white/5 border border-white/[0.07] rounded-xl px-3 py-2.5">
