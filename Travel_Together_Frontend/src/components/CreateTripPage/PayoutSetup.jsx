...
export default function PayoutSetup() {
  ...
  return (
    <div className="bg-surface-alt border-[1.5px] border-line rounded-2xl p-4 mb-4">
      <div className="flex items-center gap-2 mb-1">
        <Wallet size={14} className="text-accent" />
        <span className="text-[13px] font-bold text-ink">Where you'll get paid</span>
      </div>
      ...
      {current?.configured && !editing ? (
        <div className="flex items-center justify-between bg-surface-alt border border-line rounded-xl px-3 py-2.5">
          ...
          <button onClick={() => setEditing(true)}
            className="text-[11px] text-accent font-semibold cursor-pointer bg-transparent border-none">
            Change
          </button>
        </div>
      ) : (
