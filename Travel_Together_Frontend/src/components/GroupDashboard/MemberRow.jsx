  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[2200] flex justify-center p-3 pb-6"
        ...
        <div className="bg-surface border border-line rounded-3xl w-full max-w-sm shadow-[0_-8px_40px_rgba(0,0,0,0.5)] overflow-hidden">
          <div className="flex justify-center pt-3 pb-1">
            <div className="w-8 h-1 rounded-full bg-white/20" />
          </div>
          <div className="px-5 pt-2 pb-4">
            <p className="text-[11px] font-bold tracking-[.08em] uppercase text-ink-mute mb-0.5">Navigate to</p>
            <p className="text-[15px] font-semibold text-ink">{name}</p>
          </div>
          <div className="px-4 flex flex-col gap-2 pb-4">
            {apps.map(a => (
              <button key={a.label} onClick={() => handleApp(a.url)}
                className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-surface-alt border border-line text-left cursor-pointer hover:border-accent transition-colors w-full">
                <span className="text-xl">{a.icon}</span>
                <span className="text-[13px] font-semibold text-ink">{a.label}</span>
                <ChevronRight size={14} className="text-ink-mute ml-auto" />
              </button>
            ))}
            {canShare && (
              <button onClick={handleShare}
                ...
                <span className="text-[13px] font-semibold text-ink">Share location</span>
                <ChevronRight size={14} className="text-ink-mute ml-auto" />
              </button>
            )}
            <button onClick={handleCopy}
              ...
              <span className="text-[13px] font-semibold text-ink">{copied ? "Copied!" : "Copy link"}</span>
              {!copied && <ChevronRight size={14} className="text-ink-mute ml-auto" />}
            </button>
          </div>
          <div className="px-4 pb-5">
            <button onClick={onClose}
              className="w-full py-3 rounded-2xl bg-surface-alt border border-line text-[13px] font-semibold text-ink-mute cursor-pointer hover:bg-surface-alt transition-colors">
              Cancel
            </button>
          </div>
        </div>
...
export default function MemberRow({ m, isChief, tripId, isTripLive, onRemove, onMessage, onLocate, onViewProfile }) {
  ...
  return (
    <>
      <div className="bg-surface-alt border border-line rounded-2xl overflow-hidden transition-colors duration-150 hover:border-accent/20">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-1">
              <button
                ...
                className="text-[13px] font-bold text-ink truncate bg-transparent border-none p-0 cursor-pointer hover:text-accent transition-colors text-left"
              >{m.name}</button>
            ...
            <div className="flex items-center gap-2">
              {m.lat != null
                ...
                : <span className="text-[10px] text-ink-mute flex items-center gap-1"><MapPin size={9} />No location</span>
              }
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <div title={m.checkedIn ? "Checked in" : "Not checked in"}>
              {m.checkedIn
                ...
                : <Clock size={15} className="text-ink-mute" />
              }
            </div>
            <ChevronDown size={13} className={`text-ink-mute transition-transform duration-200 ${expanded ? "rotate-180" : ""}`} />
          </div>
        </div>

        {expanded && (
          <div className="px-3 pb-3 pt-2 border-t border-line flex gap-2 flex-wrap">
            <button onClick={e => { e.stopPropagation(); onMessage?.(m.user_id); }}
              className="flex-1 py-1.5 rounded-xl border border-line bg-surface-alt text-ink-soft text-[11px] font-semibold cursor-pointer flex items-center justify-center gap-1.5 hover:bg-surface-alt transition-colors min-w-[70px]">
            ...
            <button
              ...
              className={`flex-1 py-1.5 rounded-xl border text-[11px] font-semibold flex items-center justify-center gap-1.5 transition-colors min-w-[70px]
                ${isTripLive && m.lat != null
                  ...
                  : "border-line bg-surface-alt text-ink-mute cursor-not-allowed"}`}
            >
      ...
      {showConfirm && (
        <>
          ...
          <div className="fixed inset-0 z-[1900] flex items-center justify-center p-4 pointer-events-none">
            <div className="bg-surface border border-line rounded-3xl w-full max-w-[360px] shadow-[0_20px_60px_rgba(0,0,0,0.5)] pointer-events-auto p-6"
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-[14px] font-bold text-ink">Remove {m.name.split(" ")[0]}?</p>
                  <p className="text-[11px] text-ink-mute">They will lose access to the trip and group chat.</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setShowConfirm(false)}
                  className="flex-1 py-2.5 rounded-xl bg-surface-alt border border-line text-[13px] font-semibold text-ink-soft hover:bg-surface-alt transition-all cursor-pointer">
                  Cancel
                </button>
                <button onClick={() => { setShowConfirm(false); onRemove?.(m.user_id, m.name); }}
                  className="flex-1 py-2.5 rounded-xl bg-red-500/80 text-ink text-[13px] font-bold cursor-pointer hover:bg-red-500 transition-all">
                  Remove
                </button>
              </div>
            </div>
